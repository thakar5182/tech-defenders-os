...
router.post('/forgot', loginLimiter, async (req, res) => {
  ...
  // PUBLIC_APP_URL may point at /app.html. Password resets must always open
  // the unauthenticated login page at the deployment origin, never the app.
  let appOrigin = `${req.protocol}://${req.get('host')}`;
  try { appOrigin = new URL(String(process.env.PUBLIC_APP_URL || appOrigin)).origin; } catch (_) {}
  const resetUrl = `${appOrigin}/login?reset_token=${encodeURIComponent(token)}`;
