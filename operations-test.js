...
(async () => {
  ...
  try {
    ...
    const mobileProfile = await request(port, 'GET', '/api/auth/me', null,
      { Cookie: '', Authorization: `Bearer ${mobileLogin.json.token}`, 'X-TD-Client': 'mobile' });
    check('mobile bearer token authenticates without a browser cookie',
      mobileProfile.status === 200 && mobileProfile.json.user.email === 'admin@techdefenders.in');
