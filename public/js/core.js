...
const Core = {
  ...
  forceMfaSetup() {
    let secret = '';
    const modal = this.openModal({ title: 'Secure administrator account', closable: false, body: `<p class="muted">Administrator accounts require free Authenticator 2FA. Confirm your password, then add Tech Defenders OS in Google Authenticator, Microsoft Authenticator or Authy.</p><form id="mfa-setup-form" class="form-grid" style="margin-top:14px"><label class="field"><span>Current password</span><input name="password" type="password" autocomplete="current-password" required></label><button class="btn btn-outline" type="button" id="mfa-generate">Generate setup key</button><div id="mfa-secret" class="credential-code" hidden></div><label class="field"><span>6-digit authenticator code</span><input name="code" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" disabled required></label></form>`, footer: '<button class="btn btn-gold" type="submit" form="mfa-setup-form" disabled id="mfa-enable">Enable 2FA</button>' });
