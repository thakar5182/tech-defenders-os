...
(() => {
  ...
  function customerView(data) {
    const quotes = data.quotations?.length ? data.quotations.map(item => `<div class="item stack"><div class="item-head"><div><b>${esc(item.number)}</b><small>${date(item.date)} · ${money(item.totals?.grandTotal)}</small></div>${status(item.status)}</div>${(item.portalDiscussion||[]).length?`<div class="conversation">${item.portalDiscussion.map(row=>`<div class="message ${row.side==='customer'?'mine':''}"><b>${esc(row.by)}</b><p>${esc(row.message)}</p><small>${date(row.at)}</small></div>`).join('')}</div>`:''}<form class="quote-discussion" data-id="${item.id}"><input name="message" required maxlength="1500" placeholder="Ask a question about this quotation"><button class="btn outline">Send</button></form><div class="action-row"><button class="btn outline download" data-path="/quotations/${item.id}/pdf" data-name="${esc(item.number)}.pdf">Download PDF</button>${['draft', 'sent'].includes(item.status) ? `<button class="btn gold quote-decision" data-id="${item.id}" data-decision="accepted">Accept</button><button class="btn outline quote-decision" data-id="${item.id}" data-decision="rejected">Reject</button>` : ''}</div></div>`).join('') : empty('quotations');
    ...
    return `${hero(data)}<section class="grid cols"><article class="card"><h2>Quotations</h2>${quotes}</article><article class="card"><h2>Invoices & payment</h2>${invoices}</article><article class="card wide"><h2>Projects, milestones & tasks</h2>${projects}</article><article class="card wide"><h2>Support conversations</h2>${tickets}<form id="ticket-form" class="panel-form"><h3>Open a new ticket</h3><input name="subject" required maxlength="160" placeholder="Subject"><select name="priority"><option>medium</option><option>high</option><option>low</option></select><textarea name="message" required maxlength="2000" placeholder="How can we help?"></textarea><input name="attachment" type="file" accept="application/pdf,image/png,image/jpeg,image/webp"><small class="muted">Optional attachment · maximum 1.5 MB</small><button class="btn gold">Create ticket</button></form></article><article class="card"><h2>Documents</h2>${documents}</article><article class="card"><h2>Activity history</h2>${activityList(data.activity)}</article></section>`;
  }
  ...
  function supplierView(data) {
    ...
    const orders = data.orders?.length ? data.orders.map(item => `<div class="item stack"><div class="item-head"><div><b>${esc(item.number)}</b><small>${date(item.date)} · ${money(item.totals?.grandTotal || item.total)} · delivery ${date(item.expectedDeliveryDate)}</small><small>Delivered quantity: ${Number(item.deliveredQty)||0}</small></div>${status(item.status)}</div>${['sent', 'supplier_accepted', 'supplier_rejected'].includes(item.status) ? `<div class="action-row"><button class="btn gold po-decision" data-id="${item.id}" data-decision="accepted">Accept PO</button><button class="btn outline po-decision" data-id="${item.id}" data-decision="rejected">Reject PO</button></div>` : ''}</div>`).join('') : empty('purchase orders');
    const supplierInvoices=data.invoices?.length?data.invoices.map(item=>`<div class="item"><div><b>${esc(item.number||item.invoiceNo||'Invoice')}</b><small>${date(item.date)} · ${money(item.total||item.totals?.grandTotal)}</small></div>${status(item.paymentStatus)}</div>`).join(''):empty('supplier invoices');
  ...
  function bind(data) {
    ...
    const payloadFrom=async form=>{const values=new FormData(form),file=values.get('attachment'),payload=Object.fromEntries(values);delete payload.attachment;if(file&&file.size){if(file.size>1.5*1024*1024)throw new Error('Attachment must be smaller than 1.5 MB');payload.attachment={name:file.name,data:await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=reject;reader.readAsDataURL(file);})};}return payload;};
    portal.querySelectorAll('.quote-discussion').forEach(form=>form.addEventListener('submit',event=>{event.preventDefault();run(form,()=>api(`/quotations/${form.dataset.id}/discussion`,{method:'POST',body:JSON.stringify({message:new FormData(form).get('message')})}));}));
    portal.querySelectorAll('.reply-form').forEach(form => form.addEventListener('submit', event => { event.preventDefault(); run(form, async()=>api(`/tickets/${form.dataset.id}/replies`, { method: 'POST', body: JSON.stringify(await payloadFrom(form)) })); }));
    document.getElementById('ticket-form')?.addEventListener('submit', event => { event.preventDefault(); const form = event.currentTarget; run(form, async()=>api('/tickets', { method: 'POST', body: JSON.stringify(await payloadFrom(form)) })); });
