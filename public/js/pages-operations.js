...
Pages.newEmailCampaign = () => {
  ...
  const attachmentInput = modal.el.querySelector('[name="attachment"]'); attachmentInput.onchange = () => { const file=attachmentInput.files?.[0], note=modal.el.querySelector('[data-attachment-note]'); if (!file) return note.textContent='No image selected. The selected image will be sent as an email attachment.'; note.textContent=`Ready: ${file.name} (${bytesLabel(file.size)})`; };
  modal.el.querySelector('#campaign-form').onsubmit = async event => { event.preventDefault(); const form=new FormData(event.target), customerIds=form.getAll('customerId'); if(!customerIds.length)return toast('Select recipients','Choose at least one customer','error'); const file=attachmentInput.files?.[0]; if(file && (!['image/jpeg','image/png','image/webp'].includes(file.type)||file.size>5*1024*1024))return toast('Attachment not accepted','Choose a JPG, PNG or WEBP image of 5 MB or less','error'); const body=new FormData(); body.append('customerIds',JSON.stringify(customerIds)); ['templateId','type','subject','body','scheduledAt'].forEach(key=>body.append(key,form.get(key)||'')); if(form.get('attachInvoice')==='on')body.append('attachInvoice','true'); if(file)body.append('attachment',file,file.name); const button=modal.el.querySelector('[type="submit"]');button.disabled=true;try{await Core.uploadForm('/ops/email/send',body);modal.close();toast('Email queued',`${customerIds.length} recipient(s) added${file?' with banner attachment':''}`,'success');Core.render();}catch(error){button.disabled=false;toast('Email not queued',error.message,'error');} };
};
