...
Core.route('admin/security', async () => {
  const [sessionData, eventData] = await Promise.all([Core.get('/security/sessions'), Core.get('/security/events')]);
  Pages._securitySessions = sessionData.sessions;
  const superAdmin=Core.state.user.role==='super_admin';
  document.getElementById('content').innerHTML = `${Core.pageHead('Security Centre','Active devices, optional 2FA and suspicious-login history','<button class="btn btn-danger" onclick="Pages.logoutAllDevices()">Logout all devices</button>')}<div class="kpi-grid">${Core.kpi('Active sessions',sessionData.sessions.length,'Tracked browsers and devices')}${Core.kpi('2FA',superAdmin?'Not required':(Core.state.user.mfaEnabled?'Enabled':'Required'),superAdmin?'Super Admin can sign in without authenticator':(Core.state.user.mfaEnabled?'Authenticator protected':'Complete setup now'),superAdmin||Core.state.user.mfaEnabled?'k-success':'k-danger')}${Core.kpi('Security events',eventData.events.length,'Latest 500 events')}</div><section class="section-block"><h3>Active sessions & devices</h3>${Core.table([{label:'Device',render:r=>`<b>${Core.esc((r.userAgent||'Unknown device').slice(0,80))}</b><br><small>${Core.esc(r.ip||'Unknown IP')}</small>`},{label:'Signed in',render:r=>Core.fmtDate(r.createdAt)},{label:'Method',render:r=>Core.badge(r.method||'session')},{label:'Status',render:r=>r.current?'<span class="badge b-success">Current</span>':'<span class="badge b-info">Active</span>'},{label:'',render:r=>`<button class="btn btn-outline btn-sm" onclick="Pages.revokeDevice('${r.id}',${r.current?'true':'false'})">Revoke</button>`}],sessionData.sessions,{emptyTitle:'No active sessions'})}</section><section class="section-block"><h3>Login and security history</h3>${Core.table([{label:'When',render:r=>Core.fmtDate(r.createdAt)},{label:'Event',render:r=>Core.badge(r.type)},{label:'Account',key:'email'},{label:'IP',key:'ip'},{label:'Details',key:'details'}],eventData.events,{emptyTitle:'No security events'})}</section>`;
});
Pages.revokeDevice = async (id,current) => { if (!await Core.confirm('Revoke this device session immediately?','Revoke session')) return; await Core.post('/security/sessions/'+id+'/revoke',{}); if(current) location.href='/login'; else Core.render(); };
Pages.logoutAllDevices = async () => { if (!await Core.confirm('Sign out this account from every browser and device?','Logout all devices')) return; await Core.post('/security/logout-all',{}); location.href='/login'; };
